...
//   getSchedulesByCourse, createSchedule, updateSchedule, deleteSchedule, toggleScheduleStatus,
...
// // ── Icon components (SVG — no emoji dependency) ───────────────
// const CalendarIcon = () => (
...
//     date: '', sessionType: 'General', startTime: '',
//     endTime: '', location: '', activeSlots: '', teacherSearch: '',
//   })
...
//       // Backend returns { success, schedules: [...] }
...
//     if (!form.date || !form.startTime || !form.endTime || !form.activeSlots) {
//       alert('Please fill in Date, Start Time, End Time and Active Slots.')
//       return
//     }
//     try {
...
//         teacher:     form.teacherSearch || undefined,
//       })
//       setForm({ date: '', sessionType: 'General', startTime: '', endTime: '', location: '', activeSlots: '', teacherSearch: '' })
//       load()
//     } catch (e) { console.error('createSchedule:', e) }
//   }
...
//     try {
//       await updateSchedule(editSession._id, {
//         startTime:   editSession.startTime,
//         endTime:     editSession.endTime,
//         activeSlots: Number(editSession.activeSlots),
//       })
//       setEdit(null)
//       load()
//     } catch (e) { console.error('updateSchedule:', e) }
//   }
...
//           <div>
//             <h2>Manage Course Dates</h2>
//             <p className="sched-course-name">{course.title}</p>
//           </div>
...
//                 <input type="checkbox" /> Bulk Upload
//               </label>
//             </div>
...
//                 <input type="number" value={form.activeSlots} placeholder="e.g., 20"
...
//                 onChange={e => setForm(f => ({ ...f, teacherSearch: e.target.value }))}
//                 placeholder="Search teachers by name or email…"
//               />
//               <p className="field-hint">Search and select a teacher to conduct this session</p>
//             </div>

//             <button className="red-btn" style={{ marginTop: 14 }} onClick={handleAddDate}>
//               + Add Date
//             </button>
//           </div>
...
//                 ? <p className="empty-row" style={{ padding: '24px 0' }}>No sessions scheduled yet.</p>
//                 : Object.keys(grouped).sort().map(date => (
//                   <div key={date} className="sched-date-group">
//                     <div className="sched-date-row">
//                       <strong>{fmtDate(date)}</strong>
//                       <span className="sched-session-count">
//                         {grouped[date].length} session{grouped[date].length > 1 ? 's' : ''} available
//                       </span>
...
//                           {s.activeSlots}
//                           <br />
//                           <span className="sched-slots-label">Active slots</span>
//                         </span>

//                         {/* Edit button */}
//                         <button className="sched-edit-btn" onClick={() => setEdit({ ...s })}>
//                           Edit
//                         </button>
...
//           <button className="grey-btn" disabled>Save New Dates (0)</button>
//           <button className="cancel-btn" onClick={onClose}>Close</button>
//         </div>
//       </div>

//       {/* ── Edit Session Sub-modal ── */}
//       {editSession && (
...
//                 <div className="sched-field">
//                   <label>Start Time</label>
//                   <input type="time" value={editSession.startTime}
//                     onChange={e => setEdit(s => ({ ...s, startTime: e.target.value }))} />
//                 </div>
//                 <div className="sched-field">
//                   <label>End Time</label>
//                   <input type="time" value={editSession.endTime}
...
//               <div className="sched-field" style={{ marginTop: 12 }}>
//                 <label>Active Slots</label>
//                 <input type="number" value={editSession.activeSlots}
//                   onChange={e => setEdit(s => ({ ...s, activeSlots: e.target.value }))} />
//               </div>
//             </div>
...
// // CATEGORY MODAL
// // ══════════════════════════════════════════════════════════════
// const CategoryModal = ({ onClose, categories, onRefresh }) => {
...
//             <p className="page-sub">Add, edit, or remove course categories</p>
//           </div>
//           <button className="modal-close-btn" onClick={onClose}>✕</button>
//         </div>
...
//           />
//           <button className="red-btn" onClick={handleAdd}>+ Add</button>
//         </div>
...
//             : categories.map(cat => (
//               <div key={cat._id} className="cat-list-row">
...
//         <p className="cat-footer-note">
//           Total Categories: {categories.length} • Drag to reorder (order appears on front page)
//         </p>
//       </div>
//     </div>
//   )
...
//               <div className="sched-field span-2">
...
//               </div>
//               <div className="sched-field">
//                 <label>Course Code / URL Slug</label>
//                 <input value={form.urlSlug} onChange={e => setF('urlSlug', e.target.value)}
//                   placeholder="e.g., forklift-licence" />
//                 <p className="field-hint">Page URL: /course/{form.urlSlug || 'your-slug'}</p>
//               </div>
...
//                 <input value={form.certificateValidity} onChange={e => setF('certificateValidity', e.target.value)}
...
//                         checked={form.pricingType === t}
//                         onChange={() => setF('pricingType', t)} />
//                       {t}
//                     </label>
//                   ))}
//                 </div>
...
//                   onChange={e => setF('comboEnabled', e.target.checked)} />
...
//                   <input value={form.comboDescription}
//                     onChange={e => setF('comboDescription', e.target.value)}
//                     placeholder="Describe combo package" />

//                   <label>Combo Price (₹)</label>
//                   <input type="number" value={form.comboPrice}
//                     onChange={e => setF('comboPrice', e.target.value)} placeholder="78" />

//                   <label>Combo Duration (days)</label>
//                   <input type="number" value={form.comboDuration}
//                     onChange={e => setF('comboDuration', e.target.value)} placeholder="1" />

//                   <div className="combo-preview">
//                     <h3>Combo Preview</h3>
//                     <p><strong>Package:</strong> {form.comboDescription}</p>
//                     <p><strong>Price:</strong> ₹{form.comboPrice}</p>
//                     <p><strong>Duration:</strong> {form.comboDuration} days</p>
//                   </div>
//                 </>
//               )}
//             </div>
...
//       // courseController returns { success, courses: [...] }
//       setCourses(cRes.data.courses || [])
//       // categoryServices returns { categories: [...] }
...
//     try { await toggleCourseStatus(id); loadAll() }
//     catch (e) { console.error('toggleStatus:', e) }
//   }

//   const handleDelete = async (id) => {
//     if (!window.confirm('Are you sure you want to delete this course? This cannot be undone.')) return
//     try { await deleteCourse(id); loadAll() }
...
//           <button className="outline-btn" onClick={() => setCatModal(true)}>
//             🏷 Manage Categories
//           </button>
//           <button
//             className="red-btn"
//             onClick={() => { setEditCourse(null); setCModal(true) }}
//           >
//             + Create New Course
//           </button>
//         </div>
...
//             Find courses by name or course code
//           </p>
...
//           ? (
//             <div className="section-card">
//               <p className="empty-row">No courses found.</p>
//             </div>
//           )
...
//                   {/* Drag handle */}
//                   <div className="course-list-drag">⠿</div>
...
//                     <div className="course-list-code">
...
//                       <div className="course-list-enrolled">
...
//                     <button
//                       className="action-icon-btn action-schedule"
//                       title="Manage Dates"
...
//           course={editCourse}
//           categories={categories}
//           onClose={() => { setCModal(false); setEditCourse(null) }}
//           onSaved={loadAll}
//         />
//       )}
