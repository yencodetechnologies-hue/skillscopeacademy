...
function AssessmentModal({ record, onClose, onRefresh }) {
  ...
  const handleSaveDate = async () => {
    try {
      setSaving(true);
      // Convert datetime-local string (browser local time) to UTC ISO before sending
      // so the backend stores the exact moment the user picked regardless of server timezone.
      const utcIso = new Date(newDate).toISOString();
      const res = await fetch(`${API_URL}/api/flow/LLN-date`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ flowId: record.id, newDate: utcIso })
      });
      if (res.ok) {
        setIsEditing(false);
        if (onRefresh) onRefresh();
