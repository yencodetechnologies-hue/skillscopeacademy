...
function AccordionCard({ title, defaultOpen = true, children }) {
  ...
  return (
    <div className={`cdp-card cdp-accordion ${open ? "is-open" : ""}`}>
      <button
        type="button"
        ...
        aria-expanded={open}
      >
        <span className="cdp-accordion-title-wrap">
          <span className="cdp-accordion-accent"></span>

          <span className="cdp-accordion-title">{title}</span>
        </span>

        <span className="cdp-accordion-icon">
          <ChevronDown
            className={`cdp-accordion-chevron ${open ? "is-open" : ""}`}
            size={18}
            aria-hidden="true"
          />
        </span>
      ...
      <div
        className={`cdp-accordion-panel ${open ? "is-open" : "is-collapsed"}`}
      >
        <div className="cdp-accordion-panel-inner">{children}</div>
      </div>
    </div>
  );
}
...
function CourseDetails() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => {
        if (fetchSlug && fallbackId) {
          axios
            .get(`${API_URL}/api/courses/${fallbackId}`)
    ...
    axios
      .get(ACTIVE_COURSES_URL(API_URL))
  ...
  if (courseUnavailable) {
    return (
      <section>
        <CourseNavbar courses={courses} />
        <div
          className="cdp-loading"
  ...
  const HeroPriceCard = () => (
    ...
    ====================================================== */}
      {isSlbl && (
        <div className="cdp-price-content">
          {/* Header */}
          <div className="cdp-price-header">
            <div>
              <span className="cdp-price-eyebrow">CHOOSE YOUR LICENSE</span>

              <h3 className="cdp-price-heading">Select your training option</h3>
            </div>

            <span className="cdp-price-secure">✓ Secure</span>
          </div>

          {/* License Options */}
          <div className="cdp-license-options">
            {/* Single License */}
            <div className="cdp-license-option">
              <div className="cdp-license-option-top">
                <div className="cdp-license-number">01</div>

                <div>
                  <div className="cdp-pc-slbl-label">Single License</div>

                  <div className="cdp-license-description">SL or BL</div>
                </div>
              </div>

              <div className="cdp-license-price">
                <span className="cdp-price-now">${slVariant?.price ?? 0}</span>

                {slVariant?.original &&
                  slVariant.original > slVariant.price && (
                    <span className="cdp-price-old">${slVariant.original}</span>
                  )}
              </div>

              <button
                className="cdp-btn-book cdp-btn-single"
                onClick={() => openBooking("sl", true)}
              >
                <span>Book {slVariant?.label || "SL or BL"}</span>

                <span className="cdp-btn-arrow">→</span>
              </button>
            </div>

            {/* Both Licenses */}
            <div className="cdp-license-option cdp-license-featured">
              <span className="cdp-best-value">BEST VALUE</span>

              <div className="cdp-license-option-top">
                <div className="cdp-license-number">02</div>

                <div>
                  <div className="cdp-pc-slbl-label">Both Licenses</div>

                  <div className="cdp-license-description">SL + BL</div>
                </div>
              </div>

              <div className="cdp-license-price">
                <span className="cdp-price-now">
                  ${slblVariant?.price ?? 0}
                </span>

                {slblVariant?.original &&
                  slblVariant.original > slblVariant.price && (
                    <span className="cdp-price-old">
                      ${slblVariant.original}
                    </span>
                  )}
              </div>
              ...
              <button
                className="cdp-btn-book cdp-btn-featured"
                onClick={() => openBooking("slbl", true)}
              >
                <span>Book {slblVariant?.label || "SL + BL"}</span>
    ...
    ====================================================== */}
      {isExperience && (
        <div className="cdp-price-content">
          <div className="cdp-price-header">
            <div>
              <span className="cdp-price-eyebrow">TRAINING OPTIONS</span>

              <h3 className="cdp-price-heading">
          ...
          {/* Main Price */}
          <div className="cdp-main-price">
            <div className="cdp-main-price-label">Starting from</div>
          ...
          {/* Experience Buttons */}
          <div className="cdp-experience-options">
            <button
              className="cdp-experience-option cdp-experience-primary"
              onClick={() => openBooking("with-experience", true)}
            >
              <span className="cdp-experience-icon">✓</span>

              <span className="cdp-experience-info">
                <strong>With Experience</strong>

                <small>${course.withExperiencePrice}</small>
              </span>
            ...
            <button
              className="cdp-experience-option"
              onClick={() => openBooking("without-experience", true)}
            >
              <span className="cdp-experience-icon">+</span>

              <span className="cdp-experience-info">
                <strong>Without Experience</strong>

                <small>${course.withoutExperiencePrice}</small>
              </span>
    ...
    ====================================================== */}
      {!isSlbl && !isExperience && (
        <div className="cdp-price-content">
          <div className="cdp-price-header">
            <div>
              <span className="cdp-price-eyebrow">COURSE ENROLMENT</span>

              <h3 className="cdp-price-heading">Reserve your training</h3>
            </div>
          ...
          {/* Price */}
          <div className="cdp-main-price">
            <div className="cdp-main-price-label">Course fee</div>
          ...
          {/* Main CTA */}
          <button
            className="cdp-btn-book cdp-btn-main"
            onClick={() => openBooking(null, true)}
          >
            <span>Book Now — Pick a Date</span>
    ...
    ====================================================== */}
      ...
      >
        <span className="cdp-voc-icon">↻</span>

        <span>
          Already Trained?
          <strong> Book VOC</strong>
        </span>

        <span className="cdp-voc-arrow">→</span>
      </button>
    ...
    <div className="cdp-sb-card">
      ...
      {isSlbl && (
        <>
          ...
          <div className="cdp-sb-note">
            All inclusive · SafeWork NSW card fee included
          </div>
          <button
            className="cdp-experience-option cdp-experience-primary"
            onClick={() => openBooking("sl", true)}
          >
            ${slVariant?.price ?? 0} &nbsp; Book{" "}
            {slVariant?.label || "SL or BL"}
          </button>
          <button
            className="cdp-experience-option cdp-experience-primary"
            onClick={() => openBooking("slbl", true)}
          >
            ${slblVariant?.price ?? 0} &nbsp; Book{" "}
            {slblVariant?.label || "SL + BL"}
          </button>
        </>
      )}
      ...
      {isExperience && (
        <>
          ...
          <button
            className="cdp-sb-card-btn cdp-experience-primary"
      ...
      {!isSlbl && !isExperience && (
        <>
          ...
          <button className="cdp-sb-btn-main" onClick={() => openBooking()}>
      ...
      <button
        ...
        onClick={() =>
          navigate(
            `/voc?courseId=${course._id}${fromPortal ? "&fromPortal=true" : ""}`,
          )
        }
      >
      ...
      <ul className="cdp-sb-mini-list">
        ...
        <li>
          <span>✓</span> No prior experience required
        </li>
        <li>
          <span>✓</span> Nationally valid in all states
        </li>
      </ul>
    </div>
  );

  // Map each DB quick-fact key to its lucide icon (icons aren't stored
  // in the DB for this design — only val/label are admin-editable).
  const QUICK_FACT_ICON_MAP = {
    duration: Clock3,
    classHours: CalendarClock,
    location: MapPin,
    rto: Award,
    certificate: BadgeCheck,
    recognition: MapPinned,
  };

  // Course-specific overrides for keys that should reflect this particular
  // course rather than the generic DB default.
  const qfOverrides = {
    duration: course?.trainingDuration || null,
  ...
  const displayFacts = quickFacts.map((f) => ({
    ...
    Icon: QUICK_FACT_ICON_MAP[f.key] || Clock3,
  }));
  ...
  return (
    <div className="cdp">
            {/* =========================
    HERO FRAME
    Wraps the navbar + hero background inside one rounded,
    padded "card" — matches the reference design where the
    whole top section has a curved outer border and the
    navbar floats as a white rounded pill inside it.
========================= */}
      <div className="cdp-hero-frame">

        {/* Navbar floats on top of the hero image, inset from the edges */}
        <div className="cdp-floating-navbar">
          <CourseNavbar courses={courses} />
        </div>
      ...
      MODERN COURSE HERO
  ========================= */}
        <section className="cdp-modern-hero">
          {/* =====================================================
          BACKGROUND IMAGE
      ====================================================== */}
          {course?.image && (
            <img
              className="cdp-modern-hero-bg"
              src={cdnImage(course.image, { w: 1800 })}
              alt={course?.title || ""}
              loading="eager"
              fetchPriority="high"
              decoding="async"
          ...
          {/* Background overlay */}
          <div className="cdp-modern-overlay" />

          {/* Decorative glows */}
          <div className="cdp-glow cdp-glow-one" />
          <div className="cdp-glow cdp-glow-two" />

          {/* =====================================================
          HERO CONTAINER
      ====================================================== */}
          <div className="cdp-modern-container">
            <div className="cdp-modern-content">
              {/* =================================================
                  LEFT CONTENT
              ================================================== */}
              <div className="cdp-modern-left">
                {/* Category */}
                <div className="cdp-modern-category">
                  <span className="cdp-category-dot" />
                ...
                {/* Course Title */}
                <h1 className="cdp-modern-title">{course?.title}</h1>

                {/* View Description */}
                <div className="cdp-description-action">
                  <button
                    type="button"
                    className="cdp-view-description"
                    onClick={() => {
                      document
                  ...
                  >
                    <span>View Course Description</span>

                    <span className="cdp-description-arrow">→</span>
                  </button>
                </div>

                {/* Trust Indicators */}
                <div className="cdp-modern-trust">
                  <div className="cdp-trust-item">
                    <span className="cdp-trust-icon">
                      <ShieldCheck size={34} strokeWidth={1.8} />
                    </span>

                    <span className="cdp-trust-text">Nationally Recognised</span>
                  </div>
                  ...
                  <div className="cdp-trust-item">
                    ...
                    <span className="cdp-trust-text">Accredited Training</span>
                  </div>

                  <div className="cdp-trust-item">
                    <span className="cdp-trust-icon">
                      <FileBadge2 size={34} strokeWidth={1.8} />
                    </span>

                    <span className="cdp-trust-text">Certificate Included</span>
                  </div>
                </div>
              </div>
              ...
              {/* =================================================
                  RIGHT PRICE CARD
              ================================================== */}
              <div className="cdp-modern-right">
                <div className="cdp-price-wrapper">
                  <HeroPriceCard />
                </div>
              </div>
            </div>
      ...
      <div className="cdp-main">
        <div className="cdp-content">
                  ...
                  <div className="cdp-sessions-list">
                    {sessions.slice(0, 4).map((s, i) => {
                      ...
                      const mon = d
                        ...
                        .toUpperCase();
                      const weekday = d.toLocaleString("en-AU", {
                        weekday: "long",
                      });
                      ...
                      return (
                        <div className="cdp-session-row" key={i}>
                          ...
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/book-now/course/${course.slug}?scheduleId=${s.scheduleId}&sessionId=${s.id}${fromPortal ? "&fromPortal=true" : ""}`,
                              )
                            }
                          >
                            Book
          ...
          {(() => {
            ...
            return (
              <div className="cdp-section cdp-description-section">
                <button
                  type="button"
                  className={`cdp-section-heading ${
                ...
                >
                  <div className="cdp-section-heading-left">
                    <span className="cdp-section-accent"></span>

                    <div>
                      <span className="cdp-section-eyebrow">
                        ABOUT THE COURSE
                      </span>
                  ...
                  <span className="cdp-description-toggle">
                    <ChevronDown
                      className={`cdp-accordion-chevron ${open ? "is-open" : ""}`}
                      size={18}
                      aria-hidden="true"
          ...
          {course?.trainingDuration && (
            <div className="cdp-section">
              ...
              <p className="cdp-section-text">
                The total duration is {course.trainingDuration}. Training and
                assessment are conducted in our training centre.
              </p>
            </div>
          )}

          {/* TRAINING OVERVIEW */}
          {Array.isArray(course?.trainingOverview) &&
            course.trainingOverview.filter(Boolean).length > 0 && (
              <div className="cdp-section cdp-overview-section">
                ...
                <div className="cdp-overview-list">
                  {course.trainingOverview.filter(Boolean).map((item, i) => (
                    <div className="cdp-overview-item" key={i}>
                      <div className="cdp-overview-number">
                        {String(i + 1).padStart(2, "0")}
                      </div>

                      <div className="cdp-overview-check">
                        <BlueTick />
                      </div>

                      <span className="cdp-overview-text">{item}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* VOCATIONAL OUTCOME */}
          {Array.isArray(course?.vocationalOutcome) &&
            course.vocationalOutcome.filter(Boolean).length > 0 &&
            (() => {
              ...
              return (
                <div className="cdp-section cdp-voc-section">
                  {/* Header */}
                  <div className="cdp-section-heading cdp-voc-heading">
                    <span className="cdp-section-accent"></span>
                  ...
                  {/* Intro */}
                  {intro && (
                    <div className="cdp-voc-intro">
                      <div className="cdp-voc-intro-icon">
                        <span>✓</span>
                      </div>
                  ...
                  {/* Outcomes */}
                  {bullets.length > 0 && (
                    <div className="cdp-voc-outcomes">
                      {bullets.map((item, i) => (
                        <div className="cdp-voc-item" key={i}>
                          <div className="cdp-voc-check">
                            <BlueTick />
                          </div>

                          <div className="cdp-voc-item-content">
                            <span className="cdp-voc-item-number">
                              OUTCOME {String(i + 1).padStart(2, "0")}
                            </span>

                            <span className="cdp-voc-item-text">{item}</span>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          {/* WHAT YOU WILL LEARN */}
          {Array.isArray(course?.outcomePoints) &&
            course.outcomePoints.filter(Boolean).length > 0 && (
              <div className="cdp-section">
          ...
          {/* PATHWAYS */}
          {Array.isArray(course?.pathways) &&
            course.pathways.filter(Boolean).length > 0 && (
              <div className="cdp-section cdp-overview-section">
                <h2 className="cdp-section-title">Pathways</h2>
                {course.pathways.filter(Boolean).map((p, i) => (
                  <p key={i} className="cdp-section-text cdp-section-italic">
                    {p}
                  </p>
                ))}
              </div>
          ...
          {/* FEES AND CHARGES */}
          {Array.isArray(course?.feesCharges) &&
            course.feesCharges.filter(Boolean).length > 0 && (
              <div className="cdp-section cdp-overview-section">
          ...
          {/* OPTIONAL */}
          {Array.isArray(course?.optionalCharges) &&
            course.optionalCharges.filter(Boolean).length > 0 && (
              <div className="cdp-section cdp-overview-section">
                <h2 className="cdp-section-title">Optional</h2>
                {course.optionalCharges.filter(Boolean).map((p, i) => (
                  <p key={i} className="cdp-section-text">
                    {p}
                  </p>
                ))}
              </div>
        ...
        <div className="cdp-sidebar">
              ...
              >
                <div className="cdp-hb-inner">
                  <img
                    src={logo}
                    alt="SafeTicks Logo"
                    className="cdp-hb-logo"
                  />
                  <h3 className="cdp-hb-title">
                    {course.handbook?.title || "CODE OF PRACTICE"}
                  </h3>
                  <div className="cdp-hb-subtitle">
          ...
          <div className="cdp-sb-card cdp-sb-card--dark">
            ...
            <p className="cdp-sb-help-text">
              Our team can answer questions about course suitability, dates, and
              group bookings.
            </p>
            <a href={ORG_PHONE_1300.tel} className="cdp-sb-btn-cyan">
              ☎ {ORG_PHONE_1300.display}
            </a>
            <a href="mailto:info@safeticks.com" className="cdp-sb-btn-ghost">
              ✉ Email us
            </a>
      ...
      <div className="cdp-sticky">
        <div className="cdp-sticky-info">
          <div>
            ...
            <div className="cdp-sticky-facts">
              📅 {course?.trainingDuration} &nbsp;·&nbsp; 📍 {course?.location}{" "}
              &nbsp;·&nbsp; 🎓
            </div>
          </div>
        ...
        <div className="cdp-sticky-btns">
          {isSlbl ? (
            ...
            <button className="cdp-sticky-book" onClick={() => openBooking()}>
              Book Now — Pick a Date
            </button>
          )}
        </div>
