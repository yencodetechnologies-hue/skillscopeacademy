...
export default function MobileLandingPage({ courses = [] }) {
  ...
  const fromPortal = useMemo(() => {
    ...
    return new URLSearchParams(window.location.search).get("fromPortal") === "true";
  }, []);
  ...
  const selectedDateSessions = useMemo(() => {
    ...
    return upcomingSessions.filter((session) => session.dateKey === selectedDateKey);
  ...
  const availableTimeSlots = useMemo(() => {
    ...
    selectedDateSessions.forEach((session) => {
      ...
      if (!map.has(key)) {
        map.set(key, { key, time: session.time, timeId: session.timeId, sessions: [] });
      }
  ...
  const selectedTimeCourses = useMemo(() => {
    ...
    const selectedSlot = availableTimeSlots.find((slot) => slot.key === selectedSessionId);
      ...
      cells.push(
        <button
          ...
          onClick={() => {
            ...
            document.getElementById("mlp-session-calendar")?.classList.remove("open");
          }}
        >
          <span>{day}</span>
  ...
  return (
    <div className="mlp-root">
      ...
      <div className="mlp-carousel">
        {filteredHeroSlides.map((s, i) => (
          ...
          >
            ...
            <div className="mlp-slide-content">
              {s.courseCode && <div className="mlp-slide-badge">{s.courseCode}</div>}
              <h2 className="mlp-slide-title">{s.title}</h2>
              <p className="mlp-slide-subtitle">Gain the skills. Get certified. Work safely.</p>
              <ul className="mlp-slide-features">
                <li><i className="fa-solid fa-shield-halved mlp-feat-ico"></i><span>SafeWork NSW Approved</span></li>
                <li><i className="fa-solid fa-users mlp-feat-ico"></i><span>10,000+ Workers Trained</span></li>
                <li><i className="fa-solid fa-location-dot mlp-feat-ico"></i><span>Sefton NSW</span></li>
              </ul>
            </div>
          </div>
        ))}
        ...
        <div className="mlp-carousel-controls">
          ...
          <div className="mlp-cbtn-group">
            <button type="button" className="mlp-cbtn mlp-prev" onClick={(e) => { e.stopPropagation(); changeSlide(-1); }} aria-label="Previous Slide">‹</button>
            <button type="button" className="mlp-cbtn mlp-next" onClick={(e) => { e.stopPropagation(); changeSlide(1); }} aria-label="Next Slide">›</button>
          </div>
        </div>
        ...
        <div className="mlp-slide-actions">
          ...
          <button type="button" className="mlp-btn-secondary" onClick={(e) => { e.stopPropagation(); navigate("/voc"); }}>
            <i className="fa-regular fa-file-lines"></i> Book VOC
          </button>
        </div>
      </div>

      <div className="mlp-trust-strip">{trustPills.map((p, i) => <div key={i} className="mlp-trust-pill">{p}</div>)}</div>

      <div className="announcement-bar">
        <p>🔥 SUNDAY CLASSES AVAILABLE • ENROLL NOW • LIMITED SEATS 🔥 NATIONALLY RECOGNIZED CERTIFICATES</p>
      </div>
      ...
      <div className="mlp-section mlp-sessions-section">
        ...
        {loadingSessions? (
          <div className="mlp-session-loading">Loading available sessions...</div>
          ...
          <div className="mlp-availability-wrapper">
            <div className="mlp-course-selector">
              ...
              <div className="mlp-course-select-wrapper">
                <select className="mlp-course-select" value={selectedCourseId} onChange={(e) => handleCourseSelect(e.target.value)}>
                  <option value="">Choose a course</option>
                  {coursesWithSessions.map((c) => (
                    <option key={c.id} value={c.id}>{c.courseCode? `${c.courseCode} — ${c.title}` : c.title}</option>
                  ))}
                </select>
            ...
            {selectedCourseId && (
              <>
                <div className="mlp-date-selector">
                  ...
                  <div className="mlp-date-input-wrapper">
                    ...
                    <input
                      type="text"
                      readOnly
                      value={selectedDateKey? formatSelectedDate(selectedDateKey) : ""}
                      placeholder="Choose a date"
                      className="mlp-date-input"
                      onClick={() => document.getElementById("mlp-session-calendar")?.classList.toggle("open")}
                    />
                    <span className="mlp-date-chevron">▾</span>
                  </div>

                  <div id="mlp-session-calendar" className="mlp-session-calendar">
                    <div className="mlp-calendar-header">
                      <button type="button" onClick={() => changeCalendarMonth(-1)}>‹</button>
                      <strong>{calendarMonth.toLocaleDateString("en-AU", { month: "long", year: "numeric" })}</strong>
                      <button type="button" onClick={() => changeCalendarMonth(1)}>›</button>
                    </div>
                    <div className="mlp-calendar-weekdays">{["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map((d) => <span key={d}>{d}</span>)}</div>
                    <div id="mlp-calendar-grid" className="mlp-calendar-grid">{renderCalendar()}</div>
                    <div className="mlp-calendar-legend"><span><i />Available dates</span></div>
                  </div>
                </div>
                  ...
                  <div className="mlp-time-section">
                    ...
                    <div className="mlp-time-slots">
                      {availableTimeSlots.map((slot) => {
                        ...
                        return (
                          <button type="button" key={slot.key} className={["mlp-time-slot", isSelected? "selected" : ""].filter(Boolean).join(" ")} onClick={() => setSelectedSessionId(slot.key)}>
                            <span className="mlp-time-icon">🕐</span>
                            <span>{slot.time || "Time available"}</span>
                            {isSelected && <span className="mlp-time-check">✓</span>}
                          </button>
                        );
                      })}
                    </div>
                        ...
                        return (
                          <div key={session.id} className="mlp-session-card">
                            <div className="mlp-card-header">
                              <h4 className="mlp-card-title">{session.courseName}</h4>
                              <button type="button" className="mlp-book-now-btn" onClick={() => handleBookNowClick(session)}>Book Now</button>
                            </div>
                            <div className="mlp-card-meta">
                              <div className="mlp-meta-items">
                                {session.date && <span className="mlp-meta-item"><span className="mlp-icon">📅</span>{new Date(session.date).toLocaleDateString("en-AU",{day:"2-digit",month:"short",year:"numeric"})}</span>}
                                <span className="mlp-meta-divider">|</span>
                                {session.location && <span className="mlp-meta-item"><span className="mlp-icon">📍</span>{session.location}</span>}
                                {session.duration && <><span className="mlp-meta-divider">|</span><span className="mlp-meta-item"><span className="mlp-icon">⏳</span>{session.duration}</span></>}
                              </div>
                              {session.priceDisplay && session.priceDisplay!== "Enquire" && <div className="mlp-card-price">{session.priceDisplay}</div>}
                            </div>
                            <div className="mlp-card-footer">
                              <span className="mlp-availability-text">{low? "Filling Fast" : "Seats Available"}</span>
                              {session.availableSlots!== undefined && <span className={`mlp-seats-badge ${low? "low" : ""}`}>{session.availableSlots} Seats Left</span>}
                            </div>
                          </div>
                        );
                      })}
                ...
                {!selectedDateKey && <div className="mlp-select-date-message">Select a highlighted date to view available time slots.</div>}
              </>
            )}
            {!selectedCourseId && <div className="mlp-select-date-message">Select a course above to see its available dates.</div>}
          </div>
        )}
      </div>
      ...
      <div className="mlp-section" id="courses">
        ...
        <div className="mlp-cat-grid">
          {categoryCards.map((cat) => (
            <div key={cat.category} className="mlp-cat-tile" onClick={() => navigate(`/all-courses?category=${encodeURIComponent(cat.category)}`)}>
              <div className="mlp-cat-tile-icon" style={cat.image? { backgroundImage: `url(${cdnImage(cat.image, { w: 160 })})` } : undefined}>
                {!cat.image && <span className="mlp-cat-tile-initial">{(cat.category || "?").charAt(0).toUpperCase()}</span>}
              </div>
              <div className="mlp-cat-tile-label">{cat.category}</div>
              <div className="mlp-cat-tile-count">{cat.count} {cat.count === 1? "course" : "courses"}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mlp-section mlp-section-enquire">
        <button className="mlp-enquire-btn" onClick={() => setShowEnquire(true)}>Enquire Now</button>
      </div>
        ...
        <div className="mlp-modal-backdrop" onClick={() => setShowEnquire(false)}>
          <div className="mlp-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mlp-modal-body">
              ...
              <button className="mlp-modal-send" onClick={() => setShowEnquire(false)}>SEND ✈</button>
      ...
      {showModal && selectedBookingSlot && courseForModal && (
        <BookingModal
          ...
          extraQueryParams={`${fromPortal? "fromPortal=true&" : ""}scheduleId=${selectedBookingSlot.scheduleId}&sessionId=${selectedBookingSlot.id}`}
        />
      )}

      <div className="mlp-sticky">
        <button className="mlp-sticky-call" onClick={() => navigate("/book-now")}>Enroll Now</button>
        <a href={ORG_PHONE_1300.wa} className="mlp-sticky-wa"><span><i className="fa-brands fa-whatsapp"></i></span></a>
      </div>
    </div>
  );
}