    ...
    {steps.map((label, i) => {
      ...
      return (
        <React.Fragment key={label}>
          ...
          {/* connector line between circles — not after the last step */}
          {i < steps.length - 1 && (
            <span className={`voc-step-connector ${done ? 'done' : ''}`} />
          )}
        </React.Fragment>
      )
    })}
...
export default function VocRenewal() {
  ...
  const prefillCourse = location.state?.prefillCourse || null
  ...
  // If a course was pre-selected from the CourseCard "Buy VOC" button, add it immediately
  const [selectedCourses, setSelectedCourses] = useState(
