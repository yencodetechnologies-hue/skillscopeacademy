...
const getVariantFromType = (type) => {
    ...
    if (
        normalized === "with-experience" ||
        normalized === "without-experience"
    ) {
        return normalized;
    }

    if (normalized === "sl" || normalized === "slbl") {
        return normalized;
    }
...
function CourseDropdown({
    ...
    const filterCourses = (courses) => {
        ...
        return courses.filter((c) =>
            ...
            String(c.courseCode || "").toLowerCase().includes(search)
        );
    };
    ...
    const selected = allCourses.find(
        (c) => String(c._id) === String(value?.courseId)
    );
  ...
  const selectedVariant =
    value?.variant ||
    selected?.__variant ||
    getVariantFromType(selectedType);

const selectedVariantData = selected
    ? getCourseVariants(selected).find(
...
const variantLabel =
    ...
    const isAgent =
        String(enrollmentType || "").toLowerCase() === "agent";
        ...
        <div className="cs-dropdown" ref={ref}>
            ...
            <div
                className={`cs-dropdown__trigger ${
                    open ? "cs-dropdown__trigger--open" : ""
                }`}
            ...
            >
                <span
                    className={
                        selected
                            ? "cs-dropdown__value"
                            : "cs-dropdown__placeholder"
                    }
                >
                    {label}
                </span>

                <span className="cs-dropdown__arrow">
                    {open ? "▲" : "▼"}
                </span>
            </div>
                ...
                <div className="cs-dropdown__menu">
                    ...
                    {Object.entries(groupedCourses).map(
                        ([category, catCourses]) => {
                            ...
                            return (
                                <div key={category}>
                                    <div className="cs-dropdown__group-label">
                                        {category}
                                    </div>

                                    {filteredCourses.flatMap(
                                        (course) => {
                                            const variants =
                                                getCourseVariants(course);
                                            ...
                                            return variants.map((v) => {
                                                ...
                                                const optionLabel = v.label
                                                    ? `${course.courseCode} - ${course.title} (${v.label})${
                                                          isAgent
                                                              ? ""
                                                              : ` - $${v.price}`
                                                      }`
...
function AddCourseButton({
    ...
    const isAgent =
        String(enrollmentType || "").toLowerCase() ===
        "agent";
...
function CalendarDatePicker({
 ...
 const formatKey = (d) => {
    ...
    const allSessions = sessionsForDate.flatMap(
        (slot) =>
            (slot.sessions || [])
                .filter(
                    (s) => s.status === "Active"
...
function LockedBookingSummary({
    ...
    return (
        <div className="cs-locked-booking">
            <p className="cs-locked-booking__badge">
                Company booking link
            </p>

            <p className="cs-locked-booking__hint">
                Your company has already selected this
                course and session. Complete your details
                to enrol.
            </p>

            <div className="cs-locked-booking__card">
                <p className="cs-locked-booking__label">
                    Course
                </p>
            ...
            {linkMeta?.remaining != null && (
                <p className="cs-locked-booking__slots">
                    {linkMeta.remaining} of{" "}
                    {linkMeta.maxUses} seat
                    {linkMeta.maxUses !== 1
                        ? "s"
                        : ""}{" "}
                    remaining on this link
                </p>
            )}
        </div>
    );
...
function CourseSelection({
    ...
    const [courses, setCourses] = useState([]);
    const [activeCourseIds, setActiveCourseIds] =
        useState(null);
    const [slots, setSlots] = useState([]);
    const [courseSlots, setCourseSlots] = useState({});
    const [collapsedCourses, setCollapsedCourses] =
        useState({});
    ...
    const urlVariant = getVariantFromType(
        bookingType
    );
    ...
    const useMultiCourseFlow =
        isCompany && !isCompanyEnroll;

    const isLockedBookingLink =
        !!(
            bookingLinkData?.token &&
            bookingLinkData?.courseId
        );
    ...
    const getCoursePrice = (course) => {
        ...
        // Explicit selected variant first
        const v =
            course.__variant ||
            urlVariant ||
            null;
        ...
        const fetchData = async () => {
            try {
                ...
                try {
                    const idsRes =
                        await axios.get(
                            `${API_URL}/api/schedules/active-course-ids`
                        );

                    setActiveCourseIds(
                        idsRes.data?.length > 0
                            ? idsRes.data
                            : null
                    );
                } catch {
                    setActiveCourseIds(null);
                }
                ...
                if (
                    ...
                    if (selected) {
                        // IMPORTANT:
                        // preserve ?type= in selected course
                        const courseWithVariant = {
                            ...selected,
                            __variant:
                                urlVariant || null,
                        };
                        ...
                        try {
                            const slotRes =
                                await axios.get(
                                    `${API_URL}/api/schedules/course/${paramCourseId}`
                                );

                            const fetchedSlots =
                                filterActiveScheduleSlots(
                                    slotRes.data
                                );
    ...
    // ─────────────────────────────────────────────
    // Load slots when selected course changes
    // ─────────────────────────────────────────────
    ...
    const handleCourseChange = async (
        ...
        // IMPORTANT:
        // Store variant inside selected course
        const selectedWithVariant = {
            ...selected,
            __variant: variant,
        };
   ...
   const groupedSlots = slots.reduce((acc, slot) => {
    ...
    const handleCompanyCourseAdd = async (
        courseId,
        variant = null
    ) => {
        if (!courseId) return;
    ...
    const handleSessionSelect = (
        ...
        if (
            ...
            alert(
                "This session is already selected!"
            );
            return;
        }
        ...
        if (alreadyTaken) {
            alert(
                "This session is already selected for another entry of the same course!"
            );
            return;
        }
...
const getCompanyGroupedSlots = (courseId) => {
    ...
    const titleWithVariant = (course) => {
        ...
        const variantData =
            getCourseVariants(course).find(
                (v) =>
                    v.variant === variant
            );

        return variantData?.label
            ? `${course.title} (${variantData.label})`
            : course.title || "";
    };
