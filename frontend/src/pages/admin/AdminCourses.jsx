...
const AdminCourses = () => {
  ...
  const onCourseDragEnd = async () => {
    ...
    // Compute new order synchronously from current courses state
    setCourses(prev => {
      const inCat  = prev.filter(c => (c.category?.name || 'Uncategorised') === catName)
      const others = prev.filter(c => (c.category?.name || 'Uncategorised') !== catName)
      const fromIdx = inCat.findIndex(c => c._id === fromId)
      ...
      // Persist inside the setState callback so we have the correct array
      API.post('/courses/reorder', { orderedIds: next.map(c => c._id) })
        .catch(e => console.error('course reorder failed', e))
  ...
  // Build grouped map and sort category keys by the DB category order
  const grouped = filtered.reduce((acc, c) => {
    const key = c.category?.name || 'Uncategorised'
    if (!acc[key]) acc[key] = []
    acc[key].push(c)
  ...
  // Sort category names by their order in the categories array (from DB)
  const sortedCatNames = Object.keys(grouped).sort((a, b) => {
    const idxA = categories.findIndex(cat => cat.name === a)
    const idxB = categories.findIndex(cat => cat.name === b)
